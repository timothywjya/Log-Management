...
export default async function SupportPage() {
  const { grouped, currentUser, role } = await getTeamData("support");
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight">
                Tim <span className="text-sky-500">Support</span>
              </h1>
              <p className="text-slate-500 text-sm font-medium mt-0.5">
                {role === "administrator" ? "Semua tim support" : "Tim support Anda"}
              </p>
            </div>
          </div>
        </header>
        ...
        <div className="space-y-6">
          {Object.keys(grouped).length === 0 ? (
            <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 py-20 text-center">
              <Headphones size={40} className="mx-auto text-slate-200 mb-3" />
              <p className="text-slate-400 font-medium">Belum ada anggota tim support</p>
            </div>
          ) : (
            Object.values(grouped).map(({ team, members }) => (
              <div key={team?.id ?? "unassigned"} className="bg-white rounded-2xl lg:rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <TeamTable members={members} currentUser={currentUser} teamType="support" />
              </div>
            ))
          )}
        </div>
